...
exports.OTAUpdateService = exports.UpdateSessionState = exports.UpdateType = void 0;
...
const device_communication_service_1 = require("./device-communication.service");
...
let OTAUpdateService = class OTAUpdateService {
    constructor(deviceCommunicationService, firmwareRepository, deviceRepository, updateSessionRepository, eventEmitter) {
    ...
    async startFirmwareUpdate(deviceId, firmwareId, options = {}) {
        ...
        if (!this.isCompatible(device, firmware)) {
        ...
        const session = this.updateSessionRepository.create({
            ...
            expectedDuration: this.estimateUpdateDuration(device, firmware.data.length, chunkSize),
    ...
    async handleChunkReceived(session, report) {
        ...
        const updatedSession = await this.updateSessionRepository.findOneOrFail({
            where: { id: session.id }
        });
    ...
    async completeUpdate(session) {
        ...
        if (session.type === UpdateType.FIRMWARE) {
            await this.deviceRepository.update({ id: session.deviceId }, { firmwareVersion: session.version });
        }
    ...
    async handleUpdateError(session, error) {
        ...
        await this.updateSessionStatus(session.id, UpdateSessionState.FAILED, error.message);
    ...
    isCompatible(device, firmware) {
        if (firmware.deviceType !== device.type) {
            return false;
        }
        return true;
    }
    estimateUpdateDuration(device, dataSize, chunkSize) {
...
exports.OTAUpdateService = OTAUpdateService = __decorate([
    ...
    __param(3, (0, typeorm_1.InjectRepository)(update_session_entity_1.UpdateSession)),
